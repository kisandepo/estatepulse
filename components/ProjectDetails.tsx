...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ projects, role, currentUser, onUpdateProject }) => {
  ...
  // Added implementation for updateInteractionStatus
  const updateInteractionStatus = (interactionId: string, newStatus: EnquiryStatus) => {
    ...
    // Update local selection state to sync the modal UI
    const updatedSelected = updatedInstruments.find(i => i.id === selectedInstrument.id);
    if (updatedSelected) {
      setSelectedInstrument(updatedSelected);
    }
  };
  ...
  const generateAiInsight = async () => {
    ...
    try {
      ...
      const prompt = `Perform a high-level real estate market analysis for project: ${project.name} in ${project.location}. 
      Stats: ${stats.totalUnits} total units, ${stats.totalInteractions} client interactions. 
      Average market quote: ₹${stats.avgRate}/sqft. 
      Recommend a sales strategy for remaining units. Concise, under 120 words.`;
  ...
  return (
    <div className="max-w-6xl mx-auto pb-16 px-1 lg:px-0">
      <div className="mb-6 lg:mb-10">
        <button 
          onClick={() => navigate('/')}
          className="flex items-center gap-2 text-slate-500 hover:text-blue-600 mb-4 transition-colors font-bold text-sm"
        >
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 px-2 py-0.5 bg-blue-50 rounded">Project Master Profile</span>
            </div>
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
            <button
              ...
              className="flex-shrink-0 flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg shadow-indigo-200 disabled:opacity-50 text-sm active:scale-95"
            >
            ...
            {role === UserRole.ADMIN && (
              <button
                ...
                className="flex-shrink-0 flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl font-bold transition-all shadow-lg text-sm active:scale-95"
              >
                <PlusIcon size={18} />
                Add Units
              </button>
      ...
      {aiInsight && (
        <div className="mb-8 bg-indigo-50 border border-indigo-100 rounded-3xl p-6 lg:p-8 relative animate-in fade-in slide-in-from-top-4">
          <button onClick={() => setAiInsight(null)} className="absolute top-4 right-4 text-indigo-300 hover:text-indigo-600">
            <XIcon size={20} />
          </button>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="bg-white border border-slate-200 p-1 rounded-xl flex gap-1 shadow-sm">
          <button 
            ...
            className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-slate-100 text-blue-600' : 'text-slate-400'}`}
          >
            <LayoutGridIcon size={18} />
          </button>
          <button 
            onClick={() => setViewMode('list')}
            className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-slate-100 text-blue-600' : 'text-slate-400'}`}
          >
            <ListIcon size={18} />
          </button>
        </div>
          ...
          {project.instruments.map(inst => {
            ...
            return (
              <div 
                ...
                className="bg-white border border-slate-200 p-4 lg:p-5 rounded-2xl cursor-pointer hover:border-blue-500 hover:shadow-xl transition-all text-center group relative active:scale-95"
              >
                ...
                <div className="flex items-center justify-center gap-1.5 text-[10px] font-bold text-slate-400">
                  ...
                  {inst.interactions.length} Logs
                </div>
                {inst.interactions.length > 0 && (
                  <div className={`absolute top-3 right-3 w-2 h-2 rounded-full ring-4 ring-white ${
                    latestStatus === EnquiryStatus.SOLD ? 'bg-emerald-500' : 
                    latestStatus === EnquiryStatus.BOOKED ? 'bg-amber-500' : 'bg-blue-500'
                  }`} />
                )}
              </div>
            )
                ...
                return (
                  <tr key={inst.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-slate-100 text-slate-600 rounded-md text-[9px] font-black uppercase tracking-wider">{inst.type}</span>
                    </td>
                    <td className="px-6 py-4 font-bold text-slate-600">₹{inst.baseRate.toLocaleString()}</td>
                    <td className="px-6 py-4">
                      {latestInter ? (
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black border tracking-wider uppercase ${getStatusBadgeClass(latestInter.status)}`}>
                          {latestInter.status}
                        </span>
                      ) : (
                        <span className="text-[9px] font-black text-slate-300 tracking-wider uppercase">AVAILABLE</span>
                      )}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <button 
                        ...
                        className="text-blue-600 hover:text-blue-700 font-black text-xs uppercase tracking-widest hover:underline"
                      >
                        Details
                      </button>
                    </td>
      ...
      {isAddInstrumentOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden max-h-[95vh] overflow-y-auto custom-scrollbar">
              ...
              <form onSubmit={handleAddInstrument} className="space-y-4">
                <div>
                  ...
                  <input
                    type="text"
                    required
                    value={newInstrument.number}
                    onChange={e => setNewInstrument({...newInstrument, number: e.target.value})}
                    placeholder="e.g. B-101"
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none transition-all font-bold text-sm shadow-sm"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Unit Category</label>
                  <select
                    value={newInstrument.type}
                    onChange={e => setNewInstrument({...newInstrument, type: e.target.value as InstrumentType})}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none transition-all font-bold text-sm bg-white shadow-sm"
                  >
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Starting Quote (₹ / sqft)</label>
                  <input
                    type="number"
                    required
                    value={newInstrument.baseRate || ''}
                    onChange={e => setNewInstrument({...newInstrument, baseRate: Number(e.target.value)})}
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-blue-500 outline-none transition-all font-bold text-sm shadow-sm"
                  />
                </div>
                <div className="flex gap-3 pt-4">
                  <button
                    ...
                    className="flex-1 px-4 py-3.5 border border-slate-200 text-slate-500 font-black rounded-xl hover:bg-slate-50 transition-colors uppercase text-[10px] tracking-widest"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 px-4 py-3.5 bg-slate-900 text-white font-black rounded-xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-300 uppercase text-[10px] tracking-widest"
                  >
                    Initialize
                  </button>
                </div>
          ...
          <div className="bg-white rounded-[1.5rem] lg:rounded-[3.5rem] w-full max-w-6xl shadow-2xl overflow-hidden h-[95vh] lg:h-auto lg:max-h-[90vh] flex flex-col">
            <div className="p-5 lg:p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              ...
              <button 
                ...
                className="p-2 text-slate-300 hover:text-slate-900 transition-colors bg-white rounded-full shadow-sm"
              >
                <XIcon size={24} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 lg:gap-16">
                ...
                <div className="space-y-8">
                  <div className="flex items-center gap-3 text-blue-600">
                    <UserPlusIcon size={24} className="flex-shrink-0" />
                    <h4 className="font-black uppercase tracking-widest text-xs">Manual Activity Log</h4>
                  </div>
                  <form onSubmit={handleAddInteraction} className="space-y-6">
                    <div className="bg-blue-50/30 p-6 rounded-3xl border border-blue-100/50">
                      <p className="text-[10px] font-black text-blue-500 uppercase tracking-widest mb-4">Handling Personnel</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input
                          type="text"
                          required
                          placeholder="Agent Name"
                          value={newInteraction.agentName}
                          onChange={e => setNewInteraction({...newInteraction, agentName: e.target.value})}
                          className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-bold bg-white"
                        />
                        <input
                          type="tel"
                          required
                          placeholder="Agent Contact"
                          value={newInteraction.agentPhone}
                          onChange={e => setNewInteraction({...newInteraction, agentPhone: e.target.value})}
                          className="w-full px-4 py-3 rounded-xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-bold bg-white"
                        />
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Prospect Details</p>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <input
                          type="text"
                          required
                          ...
                          className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-bold shadow-sm"
                        />
                        <input
                          type="tel"
                          required
                          ...
                          onChange={e => setNewInteraction({...newInteraction, customerPhone: e.target.value})}
                          className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-bold shadow-sm"
                        />
                      </div>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={newInteraction.offeredRate || ''}
                            onChange={e => setNewInteraction({...newInteraction, offeredRate: Number(e.target.value)})}
                            className="w-full pl-8 pr-4 py-3.5 rounded-2xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-black shadow-sm"
                          />
                        </div>
                        <select
                          value={newInteraction.status}
                          onChange={e => setNewInteraction({...newInteraction, status: e.target.value as EnquiryStatus})}
                          className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-bold bg-white shadow-sm"
                        >
                      ...
                      <textarea
                        ...
                        className="w-full px-5 py-3.5 rounded-2xl border border-slate-200 outline-none focus:border-blue-400 text-sm font-medium resize-none shadow-sm"
                        placeholder="Key observations from the visit..."
                      />
                    </div>
                    <button
                      type="submit"
                      className="w-full py-5 bg-slate-900 text-white font-black rounded-3xl hover:bg-slate-800 shadow-2xl shadow-slate-200 transition-all flex items-center justify-center gap-3 group active:scale-[0.98]"
                    >
                      <PlusIcon size={20} className="group-hover:rotate-90 transition-transform" />
                      <span className="uppercase tracking-[0.2em] text-[10px]">Commit Journal Entry</span>
                    </button>
                  </form>
                </div>
                ...
                <div className="bg-slate-50/50 rounded-[2.5rem] p-6 lg:p-10 border border-slate-100 flex flex-col h-full">
                  <div className="flex items-center gap-3 text-slate-700 mb-8">
                    ...
                    <h4 className="font-black uppercase tracking-widest text-xs">Lifecycle Journal</h4>
                  </div>
                    ...
                    {selectedInstrument.interactions.length === 0 ? (
                      <div className="text-center py-20">
                        <div className="w-16 h-16 bg-white rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-sm">
                          <InfoIcon className="text-slate-200" size={32} />
                        </div>
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">No history recorded yet.</p>
                      </div>
                    ) : (
                      selectedInstrument.interactions.map(log => (
                        <div key={log.id} className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm animate-in fade-in slide-in-from-right-4">
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              <div className="flex items-center gap-2 mb-1.5">
                                <h5 className="font-black text-slate-900 text-sm lg:text-base leading-none">{log.customerName}</h5>
                                <span className={`text-[8px] font-black border px-2 py-0.5 rounded-full uppercase leading-none tracking-wider ${getStatusBadgeClass(log.status)}`}>
                                  {log.status}
                                </span>
                              </div>
                              <p className="text-[11px] text-slate-400 font-bold flex items-center gap-1.5"><PhoneIcon size={12} className="text-slate-300" /> {log.customerPhone}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-sm font-black text-emerald-600 leading-none mb-1">₹{log.offeredRate.toLocaleString()}</p>
                              <p className="text-[9px] text-slate-400 font-bold uppercase">/ sqft</p>
                            </div>
                          </div>
                          
                          <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-100 flex items-center justify-between mb-4">
                            <div>
                              <p className="text-[8px] text-indigo-400 font-black uppercase tracking-widest mb-1">Handled By</p>
                              <p className="text-xs font-black text-slate-800">{log.agentName}</p>
                            </div>
                          ...
                          <div className="flex gap-2 mb-4">
                            {Object.values(EnquiryStatus).map(s => (
                              <button
                                key={s}
                                onClick={() => updateInteractionStatus(log.id, s)}
                                className={`flex-1 text-[8px] py-1.5 rounded-lg border transition-all font-black uppercase tracking-tighter ${
                                  log.status === s 
